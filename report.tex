...
\usepackage[draft]{graphicx}       % 插入图片 (draft模式不加载图片，防止卡顿)
...
\begin{titlepage}
    ...
    {\LARGE\textbf{基于改进 ResNet-18 的 CIFAR-10 图像分类}\par}
...
\begin{figure}[H]
    ...
    % 假设你保存了不同激活函数的对比图，可以使用子图或分别展示
     \includegraphics[width=0.48\textwidth]{figures/ReLU_SGD.png}
    \includegraphics[width=0.48\textwidth]{figures/Sigmoid_SGD.png}
    \caption{不同激活函数下的损失收敛曲线对比}
    \label{fig:activation_compare}
\end{figure}

\textbf{结果分析}：(在此处填写你的观察，例如：Sigmoid 收敛极慢甚至不收敛，ReLU 和 LeakyReLU 表现接近，GELU 是否有提升等。)
...
\textbf{结果分析}：(在此处填写分析，例如：Adam 在初期收敛最快，但 SGD+Momentum 在后期是否达到了更高的泛化精度。)
